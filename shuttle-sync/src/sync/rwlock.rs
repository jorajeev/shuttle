...
use shuttle_core::internal::{ExecutionState, TaskId, TaskSet, switch};
...
impl<T> shuttle_core::annotations::WithName for &RwLock<T> {
    fn with_name_and_kind(self, name: Option<&str>, kind: Option<&str>) -> Self {
        (&self.semaphore).with_name_and_kind(name, kind.or(Some("shuttle::sync::RwLock")));
        self
    }
}
